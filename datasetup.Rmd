...
setnames(emailq,"RegSaleLot","RegSaleLot")
str(emailq)
```
...
setkey(bids,RegSaleLot,USER.ID)
setkey(emailq,RegSaleLot,USER.ID)

wrkdat<-merge(emailq,bids) ##,all.x=TRUE,all.y=FALSE)

## maybe just do something like the above== assign 1 if name is in both places otherwise 0 for existence of a bid.

write.csv(wrkdat,file="Data/wrkdat.csv")
save(wrkdat,file="wrkdat.rda") 
```
